#!/usr/bin/env python3
"""
Brent Crude Front-Month Futures Ingestion via yfinance

Fetches the continuous front-month Brent crude oil futures contract ("BZ=F")
...
def fetch_brent_front_month(start_date: str = None,
                            end_date: str = None,
                            period: str = None,
                            interval: str = "1d") -> pd.DataFrame:
    """
    Fetches front-month Brent crude futures from Yahoo Finance.
...
def main():
    p = argparse.ArgumentParser(
        description="Download Brent crude front-month futures from Yahoo Finance"
    )
    ...
    if args.period:
        df = fetch_brent_front_month(period=args.period, interval=args.interval)
    else:
        start, end = args.range
        df = fetch_brent_front_month(start_date=start,
                                     end_date=end,
